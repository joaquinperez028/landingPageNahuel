...
async function generateFixedSaturdaySlots() {
  ...
  try {
    ...
    for (const saturdayDate of saturdayDates) {
      ...
      for (const timeSlot of timeSlots) {
        try {
          // CORREGIDO: Usar la misma lógica que la API para evitar offset de un día
          // Construir la fecha como local, no como UTC
          const year = saturdayDate.getFullYear();
          const month = saturdayDate.getMonth();
          const day = saturdayDate.getDate();
          
          // Crear fecha local (no UTC) para AdvisorySchedule
...
module.exports = { generateFixedSaturdaySlots }; 